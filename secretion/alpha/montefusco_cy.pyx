...
cdef double DIRECT = 0.5 #  0.5
cdef double INDIRECT = 0.2 # 0.2
...
cdef double dV_dt(double V, double m_CaL, double h_CaL, double m_CaPQ, double h_CaPQ, double m_CaT, double h_CaT, double m_Na, double h_Na, double m_K, double m_KA, double h_KA, double g_KATP, double g_L, double f_cAMP, double f_AA):
    ...
    return -((1-INDIRECT+INDIRECT*f_cAMP)*(f_AA*0.16+1)*(CaL+CaPQ+CaT)+(f_AA*0.4+1)*Na+(f_AA*0.20+1)*(K+KA)+KATP+L+SOC)/5
...
cdef double dCaC_dt(double CaM, double CaC, double CaER):
    return f*(B_m*(CaM-CaC) + p_leak*(CaER-CaC) - k_SERCA*CaC)*10.0 # change
cdef double dCaER_dt(double CaC, double CaER):
    return -f*Vol_c_Vol_er*(p_leak*(CaER-CaC) - k_SERCA*CaC)

cdef montefusco(double[::1] current_vals, double g_KATP, double g_L, double f_cAMP, double f_AA):
    V = current_vals[0]
    ...
    next_vals[0] = dV_dt(V, m_CaL, h_CaL, m_CaPQ, h_CaPQ, m_CaT, h_CaT, m_Na, h_Na, m_K, m_KA, h_KA, g_KATP, g_L, f_cAMP, f_AA)
...
def montefusco_euler(double[::1] initial_vals, double g_KATP, double g_L, double f_cAMP, double f_AA, double t, double dt):
    cdef int i
    cdef int N = int(t/dt)
    v = np.zeros((N, 15))
    c = np.zeros((N, 15))
    v[0,:] = initial_vals
    for i in range(1,N):
        next_vals, next_currents = montefusco(v[i-1], g_KATP, g_L, f_cAMP, f_AA)
